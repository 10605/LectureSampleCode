...
from merge_sort import merge_sort_file
...
class PolarBar:
    ...
    def sort_kv_pairs(self, input_tsv_path, output_tsv_path):
        ...
        record and no encoding at all.
        """
        def key_of(line):
            return self.decode(line[:line.index('\t')])

        merge_sort_file(input_tsv_path, output_tsv_path, key=key_of,
                        batch_size=self.sort_batch_size, fanin=self.sort_fanin,
                        verbose=True)
        self._mark_sorted(output_tsv_path)
