...
#    pagerank(lines, reset=args.reset, num_iterations=args.num_iterations)
    ...
    edge_lines = pl.scan_lines('../data/citeseer-graph.txt')
    # construct edges
    edges = (
        edge_lines
        .with_columns(edge=pl.col('line').str.extract_groups(r'(\S+)\s+(\S+)'))
    ...
    #n_outlinks = edges.group_by('src').len().rename(dict(len='n_outlinks')
    bar.sink_kv_pairs(edges, 'edges.tsv', key_col='src', val_col='dst')
    bar.sort_kv_pairs('edges.tsv', 'edges-sorted.tsv')
    bar.group_and_aggregate_by_key('edges-sorted.tsv', 'n_outlinks.tsv', agg=pb.count)

    # edges = edges.join(n_outlinks, on='src')
    bar.join_by_key(
        'n_outlinks.tsv', 'edges-sorted.tsv',
        'edges_with_outlinks.tsv', 
        combine_vals=lambda n, dst: f'{n}\t{dst}'
    )
    ...
    bar.sink_kv_pairs(nodes_with_dups, 'nodes-with-dups.tsv', key_col='node', val_col='score')
    bar.sort_kv_pairs('nodes-with-dups.tsv', 'nodes-sorted.tsv')
    bar.group_and_aggregate_by_key('nodes-sorted.tsv', 'init-scores.tsv', agg=pb.first)
