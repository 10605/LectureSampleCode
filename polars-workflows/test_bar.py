...
def test_sort_matches_sorted(paths, batch_size, fanin, n):
    ...
    assert read_pairs(out_path) == sorted(pairs, key=bar.BY_KEY)
...
def test_sort_of_sorted_input_is_a_noop(paths):
    in_path, out_path = paths
    pairs = sorted(sample_pairs(60), key=bar.BY_KEY)
    write_pairs(in_path, pairs)
...
def test_group_and_aggregate_matches_dict_oracle(paths, n, n_keys):
    in_path, out_path = paths
    pairs = sorted(sample_pairs(n, n_keys), key=bar.BY_KEY)
    write_pairs(in_path, pairs)

    bar.PolarBar().group_and_aggregate_by_key(in_path, out_path, list)

    expected = [(k, [v for _, v in g])
                for k, g in itertools.groupby(pairs, key=bar.BY_KEY)]
...
def test_aggregators(paths, agg):
    in_path, out_path = paths
    pairs = sorted(sample_pairs(50, n_keys=5), key=bar.BY_KEY)
    write_pairs(in_path, pairs)

    bar.PolarBar().group_and_aggregate_by_key(in_path, out_path, agg)

    groups = {k: [v for _, v in g] for k, g in itertools.groupby(pairs, key=bar.BY_KEY)}
